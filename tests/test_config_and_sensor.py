...
from twodosumi.sensors import (
    AdafruitHX711AggregateReader,
    MockReader,
    check_sensor,
    median_raw,
    moving_average,
...
class ConfigAndSensorTests(unittest.TestCase):
    ...
    def test_hx711_retries_invalid_endpoint_values(self):
        reader = object.__new__(AdafruitHX711AggregateReader)
        values = iter([-1, 0x7FFFFF, 12345])
        reader._wait_until_ready = lambda: None
        reader._read_channel_a_gain_128 = lambda: next(values)

        self.assertEqual(reader.read_raw(), 12345)

    def test_hx711_fails_after_repeated_invalid_endpoint_values(self):
        reader = object.__new__(AdafruitHX711AggregateReader)
        reader._wait_until_ready = lambda: None
        reader._read_channel_a_gain_128 = lambda: -1

        with self.assertRaisesRegex(RuntimeError, "invalid endpoint value"):
            reader.read_raw()

    def test_hx711_clock_is_lowered_when_bit_read_fails(self):
        reader = object.__new__(AdafruitHX711AggregateReader)
        clock_values = []
        reader._set_clock_pin = clock_values.append
        reader._read_data_pin = lambda: (_ for _ in ()).throw(RuntimeError("read failed"))

        with self.assertRaisesRegex(RuntimeError, "read failed"):
            reader._read_channel_a_gain_128()

        self.assertFalse(clock_values[-1])
