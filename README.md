...
The installer defaults to `APP_DIR=/home/pi/2dosumi`, `APP_USER=pi`, and port `8080`. Override them if needed:
...
and talks to the existing Raspberry Pi Flask API; the sensor and detector still
run on the Pi.

Open `android/` in Android Studio, sync Gradle, then run the `app`
configuration. On first launch, enter the Pi Web UI URL, for example
`http://raspberrypi.local:8080` or your Tailscale Serve URL, and the
`web_ui_token` from `config/secrets.json`.

If you have a local JDK and Gradle installed, you can also build from the
project root:
...
The app mirrors the Web UI:

- status polling every 3 seconds, start/stop controls, weight sparkline, and event timeline
- quick alarm toggles and scheduled alarm editing
- sensor check, zero calibration, and scale calibration
- full settings editing and webhook test
