...
def make_status(
    *,
    ...
    return RuntimeStatus(
        ...
        pending_rechecks=pending_rechecks or [],
    )
...
def read_status(path: str | Path) -> dict[str, object]:
    ...
    if not target.exists():
        return {"running": False, "message": "status file not found", "pending_rechecks": []}
    with target.open("r", encoding="utf-8") as f:
        data = dict(json.load(f))
    if data.get("pending_rechecks") is None:
        data["pending_rechecks"] = []
    return data
